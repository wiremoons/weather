...
int main(int argc, char **argv)
{
        ...
        /* if '-h' used on command line display help and exit */
        if (help) {
                show_help();
                return EXIT_SUCCESS;
        }

        /* get the full URL to use with  DarkSky web site API that includes the
         * DarkSky API key and the long/lat co-ordinates for the weather
         * forecast location */
        ...
        /* Request the weather data in JSON format providing the URL to use and
         * the temporary file to write the JSON output into */
        long web_result_code = getpage(cli_page, infile);

        /* expecting a '200' result code from DarkSky web site - print out
         * anything different so it can be investigated */
        if (web_result_code != 200) {
                fprintf(stderr,
                        "\nWARNING: DarkSky web site returned http response "
                        "code: %ld\n",
                        web_result_code);
        }

        /* Read the temporary files JSON data into a memory buffer called
         * 'source' ready for parsing  in the next step if successful */
 ...
 * and clean ups such as freeing heap memory etc. Is invoked by registering
 * 'atexit()' function at beginning of main()
 *
 * Function returns nothing and is always last part to the program to run
 */
